🌱 EcoSmart Solutions

Proyecto desarrollado para promover la ODS 13: Acción por el Clima mediante una plataforma web sostenible.

✔ Desarrollo en PHP y MySQL.
✔ Sistema de registro e inicio de sesión.
✔ Gestión de sesiones y perfiles de usuario.
✔ Control de acceso por roles (usuario y administrador).
✔ Navbar flotante con navegación dinámica.
✔ Diseño responsive con Bootstrap y CSS personalizado.
✔ Integración de logo corporativo.
✔ Secciones informativas sobre la problemática ambiental, beneficios, campañas y proyectos.
✔ Estructura modular mediante header, navbar y footer reutilizables.

Este proyecto integra conocimientos de Desarrollo Web, Aplicaciones Web Progresivas, con el objetivo de fomentar hábitos sostenibles mediante herramientas tecnológicas modernas.
